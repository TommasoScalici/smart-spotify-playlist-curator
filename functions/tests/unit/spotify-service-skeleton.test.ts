...
describe("SpotifyService - Skeleton Strategy", () => {
  ...
  beforeEach(() => {
    ...
    (SpotifyWebApi as unknown as jest.Mock).mockReturnValue(mockSpotifyApi);
  ...
  it("should execute Skeleton Strategy: Remove Non-VIPs -> Reorder VIPs -> Insert Blocks", async () => {
    ...
    const currentTracks = [
      { track: { uri: "spotify:track:A", name: "A", artists: [{ name: "Art" }], id: "1", type: "track" }, added_at: "2023-01-01" },
      { track: { uri: "spotify:track:B", name: "B", artists: [{ name: "Art" }], id: "2", type: "track" }, added_at: "2023-01-02" },
      { track: { uri: "spotify:track:C", name: "C", artists: [{ name: "Art" }], id: "3", type: "track" }, added_at: "2023-01-03" },
      { track: { uri: "spotify:track:D", name: "D", artists: [{ name: "Art" }], id: "4", type: "track" }, added_at: "2023-01-01" }, // VIP
      { track: { uri: "spotify:track:E", name: "E", artists: [{ name: "Art" }], id: "5", type: "track" }, added_at: "2023-01-04" },
    ];
    ...
    mockSpotifyApi.removeTracksFromPlaylist.mockResolvedValue({ body: { snapshot_id: "snap2" } } as any);
    mockSpotifyApi.reorderTracksInPlaylist.mockResolvedValue({ body: { snapshot_id: "snap3" } } as any);
    ...
    const expectedRemove = [{ uri: "spotify:track:B" }, { uri: "spotify:track:C" }, { uri: "spotify:track:E" }];
    expect(mockSpotifyApi.removeTracksFromPlaylist).toHaveBeenCalledWith(playlistId, expectedRemove);
    ...
    expect(mockSpotifyApi.addTracksToPlaylist).toHaveBeenNthCalledWith(1, playlistId, ["spotify:track:X", "spotify:track:Y"], { position: 1 });
    expect(mockSpotifyApi.addTracksToPlaylist).toHaveBeenNthCalledWith(2, playlistId, ["spotify:track:Z"], { position: 4 });
  });
  ...
  it("should reorder VIPs if they are out of order in skeleton", async () => {
    ...
    const currentTracks = [
      { track: { uri: "spotify:track:D", name: "D", artists: [], id: "4", type: "track" }, added_at: "2023-01-01" },
      { track: { uri: "spotify:track:A", name: "A", artists: [], id: "1", type: "track" }, added_at: "2023-01-01" },
    ];
    ...
    mockSpotifyApi.reorderTracksInPlaylist.mockResolvedValue({ body: { snapshot_id: "snap2" } } as any);
    ...
    // Move 1 -> 0.
    expect(mockSpotifyApi.reorderTracksInPlaylist).toHaveBeenCalledWith(playlistId, 1, 0, expect.any(Object));
  });
});
